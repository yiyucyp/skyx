...
private:
    ...
    */
    ...
    private:
        std::array<value_type, 2> buffers_;
        std::size_t buffers_count_;
    }; // class buffers_2

public:
    /// Constant buffer sequence.
    typedef buffers_2<asio::const_buffer>   const_buffers_type;
    /// Mutable buffer sequence.
    typedef buffers_2<asio::mutable_buffer> mutable_buffers_type;
...
{
    return std::addressof(buffers_[0]);
}
...
{
    return std::addressof(buffers_[0]) + buffers_count_;
}
...
inline cyclic_buffer::const_buffers_type cyclic_buffer::data() const
{
    ...
    {
        return const_buffers_type(
            asio::const_buffer(data_.get() + filled_start_, d),
            asio::const_buffer(data_.get(), filled_size_ - d));
    }
    return const_buffers_type(asio::const_buffer(
...
inline cyclic_buffer::mutable_buffers_type cyclic_buffer::prepared() const
{
    ...
    {
        return mutable_buffers_type(
            asio::mutable_buffer(data_.get() + nonfilled_start_, d),
            asio::mutable_buffer(data_.get(), nonfilled_size_ - d));
    }
    return mutable_buffers_type(asio::mutable_buffer(
...
cyclic_buffer::data(std::size_t max_size) const
{
    ...
    if (buffers_size > d)
    {
        return const_buffers_type(
            asio::const_buffer(data_.get() + filled_start_, d),
            asio::const_buffer(data_.get(), buffers_size - d));
    }
    return const_buffers_type(asio::const_buffer(
...
cyclic_buffer::prepared(std::size_t max_size) const
{
    ...
    if (buffers_size > d)
    {
        return mutable_buffers_type(
            asio::mutable_buffer(data_.get() + nonfilled_start_, d),
            asio::mutable_buffer(data_.get(), buffers_size - d));
    }
    return mutable_buffers_type(asio::mutable_buffer(
